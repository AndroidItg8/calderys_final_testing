...
public class SpinnerTypeListAdapter   extends ArrayAdapter<String> implements Filterable, ISpinnerSelectedView {

    private Context mContext;
    private List<TblProductType> mBackupStrings;
    private List<TblProductType> mStrings;
    private StringFilter mStringFilter = new StringFilter();

    public SpinnerTypeListAdapter(Context context, List<TblProductType> strings) {
    ...
    public String getItem(int position) {
        if (mStrings != null && position > 0)
            return mStrings.get(position).getProductType();
        else
            return null;
    }
...
//            letters.setImageDrawable(getTextDrawable(mStrings.get(position-1)));
            dispalyName.setText(mStrings.get(position-1).getProductType());
        }
        return view;
    }
...
//    }
    ...
    public TblProductType getItems(int positions) {
    ...
    public class StringFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            final ArrayList<TblProductType> filterStrings = new ArrayList<>();
            for (TblProductType text : mBackupStrings) {
                if (text.getProductType().toLowerCase().contains(constraint)) {
